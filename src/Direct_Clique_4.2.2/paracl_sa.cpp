...
int main(int argc, char **argv)
{
  ...
  if (argc < 6)
  {
    cerr << "Usage:  " << argv[0] << " <DIMACS graph> <Aggressiveness Ratio> <Min. maximum clique size> <Min. paraclique size> <Max. no. paracliques>" << endl;
  ...
  Graph orig_g(*g);
  Graph::Vertices paraclique2(orig_g);
  agg_ratio = max(2, atoi(argv[2]));
  ...
  for (i=0; i<max_num_pcs; ++i)
  {
    ...
    build_acc_paracl(*g, paraclique, agg_ratio);
    if (paraclique.size() < min_pc_size) break;

    // Second stage - expand paraclique across entire graph

    // Extract results again
    paraclique2.clear();
    Graph::Vertices::Vex_ptr vp2(paraclique);
    for (; !vp2.end(); ++vp2) paraclique2.insert(orig_g.vertex(g->label(*vp2)));

    build_acc_paracl(orig_g, paraclique2, agg_ratio);
    print_vertices_oneline(paraclique2);
    g->remove(paraclique);
  }
